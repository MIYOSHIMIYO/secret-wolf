...
async function testErrorScenarios() {
  ...
  try {
    ...
    const ws = await createWebSocket(roomId);
    const playerId = await joinRoom(ws, roomId, 'テストプレイヤー', generateId());
    
    // 無効なメッセージを送信
    ws.send(JSON.stringify({ t: "invalid_message", p: {} }));
    
    await delay(1000);
    recordTest('無効なメッセージ - エラーハンドリング', true);
    
    // 重複参加テスト
    try {
      const ws2 = await createWebSocket(roomId);
      await joinRoom(ws2, roomId, 'テストプレイヤー2', generateId());
      recordTest('重複参加 - 正常処理', true);
      ws2.close();
    } catch (error) {
      recordTest('重複参加 - エラーハンドリング', false, error.message);
    }
    
    ws.close();
    
  } catch (error) {
    recordTest('エラー・異常系シナリオテスト', false, error.message);
  }
}
...
async function testProductionScenarios() {
  ...
  try {
    ...
    recordTest('本番サーバー接続 - ヘルスチェック', healthResponse.status === 200);
    
    // HTTPS通信テスト
    const httpsTest = await makeRequest('/rooms', 'POST', { mode: 'NONE' });
    recordTest('HTTPS通信 - ルーム作成', httpsTest.status === 200);
    
    // CORS設定テスト
    const corsResponse = await makeRequest('/rooms', 'OPTIONS');
    recordTest('CORS設定 - プリフライトリクエスト', corsResponse.status === 200);
    
  } catch (error) {
    recordTest('本番環境シナリオテスト', false, error.message);
  }
}

// メイン実行関数
async function runComprehensiveTests() {
  console.log('🚀 本番環境包括的シナリオテスト開始');
