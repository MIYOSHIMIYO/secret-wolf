...
export default function Lobby() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      console.log(`[Lobby] WebSocket接続状態:`, debugInfo);
    }, 10000);
  ...
  useEffect(() => {
    const handleWsMessage = (message: any) => {
      ...
      if (t === "disband") {
        console.log("[Lobby] 解散メッセージ受信:", p);
        setDisbandOpen(true);
      }
      
      if (t === "abort") {
        console.log("[Lobby] 中断メッセージ受信:", p);
        // ゲーム中断時の処理
  ...
  useEffect(() => {
    ...
    if (room.roomId !== id) {
      console.warn("[Lobby] ルームID不一致:", { expected: id, actual: room.roomId });
      nav("/menu", { replace: true });
    }
  }, [id, room, nav]);
  ...
  const ackDisband = () => {
    console.log("[Lobby] 解散OK、静かに切断してメニューへ戻ります");
  ...
  const disband = () => {
    console.log("[Lobby] 解散確認モーダルを表示");
    setConfirmOpen(true);
  };
  
  const confirmYes = () => { 
    console.log("[Lobby] ルームを解散します");
    console.log("[Lobby] WebSocket接続状態:", getConnectionDebugInfo());
    setConfirmOpen(false); 
    const result = send("disband", {}); 
    console.log("[Lobby] 解散メッセージ送信結果:", result);
  };
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2">
                <SecondaryBtn onClick={() => {
                  console.log("[Lobby] 解散キャンセル");
                  confirmNo();
                }}>いいえ</SecondaryBtn>
                <DangerBtn onClick={() => {
                  console.log("[Lobby] 解散確認");
                  confirmYes();
                }}>はい</DangerBtn>
              </div>
            </div>
