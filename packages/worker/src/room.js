...
import { RoomStateZ } from "@secret/shared/src/gameTypes";
...
function generateUniqueNickname(baseNick, existingNicks) {
    // 最大8グラフェムに制限
...
export class RoomDO {
    ...
    async fetch(request) {
        ...
        if (request.headers.get("upgrade") === "websocket") {
            ...
            server.addEventListener("message", (ev) => this.onMessage(clientId, ev));
    ...
    async alarm() {
        ...
        if (now < (this.roomState.endsAt ?? 0))
            return;
        ...
        if (this.roomState.phase === "INPUT") {
            ...
            const activePlayers = this.roomState.players.filter((p) => p.connected);
    ...
    onClose(clientId) {
        ...
        {
            const idx = this.roomState.players.findIndex((p) => p.id === pid);
            if (idx >= 0) {
        ...
        try {
                ...
                if (!id) {
                    ...
                    if (this.roomState.isAutoRoom) {
                        ...
                        if (isFull) {
                            ...
                            ws?.send(JSON.stringify({ t: "warn", p: { code: "ROOM_FULL", msg: "このルームは満員です" } }));
                    ...
                    const existingPlayer = this.roomState.players.find((p) => p.id === id);
                    if (existingPlayer) {
            ...
            if (t === "auto") {
                ...
                if (this.roomState.phase !== "LOBBY" && this.roomState.phase !== "READY") {
                    ...
                    const ws = this.clients.get(clientId);
                    ws?.send(JSON.stringify({ t: "warn", p: { code: "ROOM_CLOSED", msg: "ゲームが既に開始されています" } }));
                ...
                if (!id) {
                    ...
                    if (this.roomState.phase === "LOBBY") {
                        const pl = this.roomState.players.find((x) => x.id === id);
                        if (pl) {
                            pl.connected = true;
            ...
            if (t === "start") {
                const pid = this.clientToPlayerId.get(clientId);
                if (this.roomState.phase !== "LOBBY" || !pid || this.roomState.hostId !== pid)
                    return;
            ...
            if (t === "modeStamp") {
                const pid = this.clientToPlayerId.get(clientId);
                if (!pid || this.roomState.phase !== "MODE_SELECT")
                    return;
                // ホストは投票できない
            ...
            if (t === "submitSecret") {
                const pid = this.clientToPlayerId.get(clientId);
                if (!pid || this.roomState.phase !== "INPUT")
                    return;
                ...
                const activePlayers = this.roomState.players.filter((p) => p.connected).length;
            ...
            if (t === "submitSubmission") {
                const pid = this.clientToPlayerId.get(clientId);
                if (!pid || this.roomState.phase !== "REVEAL")
                    return;
            ...
            if (t === "chat") {
                const pid = this.clientToPlayerId.get(clientId);
                if (!pid || this.roomState.phase !== "DISCUSS")
                    return;
            ...
            if (t === "vote") {
                const pid = this.clientToPlayerId.get(clientId);
                if (!pid || this.roomState.phase !== "VOTE")
                    return;
            ...
            if (t === "report") {
                ...
                const installId = String(p?.installId ?? "");
                if (!targetPlayerId || !installId)
                    return;
                if (targetPlayerId === pid)
                    return;
                const targetPlayer = this.roomState.players.find((p) => p.id === targetPlayerId);
                if (!targetPlayer)
                    return;
                try {
                    // レート制限チェック
            ...
            if (t === "rematch") {
                const pid = this.clientToPlayerId.get(clientId);
                if (!pid || this.roomState.phase !== "RESULT")
                    return;
                // オートマッチングではリマッチの概念は持たない
                if (this.roomState.isAutoRoom) {
                    console.log(`[rematch] isAutoRoomのため無視`);
                    return;
                }
                const player = this.roomState.players.find((p) => p.id === pid);
                if (!player || this.roomState.hostId !== pid)
                    return;
            ...
            if (t === "endGame") {
                ...
                if (this.roomState.isAutoRoom) {
                    console.log(`[endGame] isAutoRoomのため無視`);
                    return;
                }
                const player = this.roomState.players.find((p) => p.id === pid);
                if (!player || this.roomState.hostId !== pid)
            ...
            if (t === "exitGame") {
                ...
                if (!pid)
                    return;
                const p = this.roomState.players.find((x) => x.id === pid);
                if (p) {
                    p.left = true;
                    p.connected = false;
                }
            ...
            if (t === "leave") {
                ...
                const idx = this.roomState.players.findIndex((x) => x.id === pid);
                if (idx >= 0) {
                    const pl = this.roomState.players[idx];
                    if (pl.iconId) {
                        // 配列からアイコンIDを削除
            ...
            if (t === "selectMode") {
                const pid = this.clientToPlayerId.get(clientId);
                if (!pid || this.roomState.phase !== "MODE_SELECT" || this.roomState.hostId !== pid)
                    return;
                const mode = String(p?.mode || "");
                if (!mode)
                    return;
                ...
                // モードに応じたお題を設定
                const modePrompts = PROMPTS[mode] || PROMPTS.NONE;
    ...
    activeIds() {
        return this.roomState.players.filter((p) => p.connected && !p.left).map((p) => p.id);
    }
    broadcast(msg) {
    ...
    broadcastState() {
        try {
            ...
            this.broadcast({ t: "state", p: RoomStateZ.parse(stateForValidation) });
        }
        catch (error) {
