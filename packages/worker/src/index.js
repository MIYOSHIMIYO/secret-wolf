...
// 最適化されたログ出力関数
function log(env, level, message, ...args) {
...
router.get("/debug/monitoring", (req, env) => {
    const origin = req.headers.get("Origin") || undefined;
    // 認証チェック
...
router.post("/rooms", async (req, env) => {
...
// 通報送信API
router.post("/report", async (request, env) => {
    try {
        // レート制限チェック
...
async function calculateTotalPoints(env, installId, date) {
    try {
        const prefix = `report:${installId}:`;
        const reports = await env.MOD_KV.list({ prefix });
        let totalPoints = 0;
...
const modeRooms = new Map(); // mode -> [roomId1, roomId2, ...]
// ルームの有効性をチェックする関数
async function checkRoomValidity(env, roomId) {
    try {
...
// 利用可能なルームを探す関数
async function findAvailableRoom(env, mode) {
    try {
...
// 新しいルームをモードに追加する関数
function addRoomToMode(mode, roomId, env) {
...
// ルームをモードから削除する関数
function removeRoomFromMode(mode, roomId, env) {
    if (modeRooms.has(mode)) {
        const roomIds = modeRooms.get(mode);
...
async function updateModeRooms(env) {
...
// 最適化されたレート制限
function checkRateLimit(ip, endpoint, env) {
    const isProduction = env.ENVIRONMENT === "production";
...
router.post("/auto", async (req, env) => {
    try {
        // レート制限チェック
