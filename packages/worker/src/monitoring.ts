...
export class DurableObjectMonitor {
   ...
   * 接続数の監視
   */
  recordConnection(roomId: string, connectionCount: number) {
   ...
   * CPU時間の監視
   */
  recordCpuTime(roomId: string, cpuTime: number) {
    const latestData = this.monitoringData[this.monitoringData.length - 1];
    if (latestData && latestData.roomId === roomId) {
   ...
   * クォータ使用量の更新
   */
  updateQuotaUsage(usage: Partial<QuotaUsage>) {
